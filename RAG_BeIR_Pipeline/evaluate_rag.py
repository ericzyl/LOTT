# Evaluating and Comparing BERT and LOTT RAG Systems
import numpy as np
import pickle
import time
import json
...
def compute_ndcg_at_k(retrieved: List[str], relevance_scores: Dict[str, float], k: int) -> float:
    # Computing Normalized Discounted Cumulative Gain@K
    retrieved_k = retrieved[:k]
...
class RAGEvaluator:
    ...
    def print_comparison(self): # Printing Comparison Table
...
def benchmark_timing(bert_system: BERTRAGSystem, lott_system: LOTTRAGSystem,
    ...
    # Benchmarking Timings for both Systems
    print("\n" + "="*80)
